...
import { Send, Paperclip, Copy, Download, LinkIcon, Info, X } from "lucide-react"
...
export function ChatArea({ selectedGroup }) {
  ...
  const { messages, loading, error, sendMessage } = useMessages(selectedGroup?.id)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // If the teacher is not available and we got an AI response,
      // it will be automatically added to the messages through the real-time subscription
      if (!data.teacherAvailable && data.aiResponse) {
        // The AI response will be added through the real-time subscription
  ...
  const handleAvatarClick = async (otherUserId) => {
    ...
    try {
      // Create a personal group between the current user and the clicked user
      const result = await createGroup({
        subject_name: "Direct Message",
        semester_id: selectedGroup?.semester_id || "1", // Default to semester 1 if not available
        group_type: "personal",
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center p-6">
          <div className="bg-muted p-6 rounded-lg">
            ...
            <p className="text-muted-foreground">Select a group from the sidebar to start chatting</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="border-b py-3 px-4 flex justify-between items-center bg-background/95 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Avatar className="h-9 w-9">
            <AvatarImage src={selectedGroup.avatar || "/placeholder.svg?height=36&width=36"} alt={selectedGroup.name} />
            <AvatarFallback>{selectedGroup.name.charAt(0).toUpperCase()}</AvatarFallback>
          </Avatar>
          <div>
            <h1 className="font-semibold text-lg leading-tight">{selectedGroup.name}</h1>
              ...
              selectedGroup.teachers.map((teacher) => {
                return teacher.is_available ? (
                  <span key={teacher.id} className="inline-flex items-center">
                    ...
                    <span className="text-xs text-muted-foreground">Teacher online</span>
                  </span>
                ) : (
                  <span key={teacher.id} className="inline-flex items-center">
                    <span className="h-2 w-2 rounded-full bg-red-500 mr-1"></span>
                    <span className="text-xs text-muted-foreground">AI assistant active</span>
                  </span>
                )
              })}
          </div>
              ...
              {Object.keys(messageGroups).map((date) => (
                <div key={date} className="space-y-4">
                  ...
                  {messageGroups[date].map((message, index) => {
                    ...
                    const showAvatar = index === 0 || messageGroups[date][index - 1].sender_id !== message.sender_id
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={cn("max-w-[75%]", isCurrentUser ? "order-1" : "order-2")}>
                          {showAvatar && (
                            <div className={cn("text-xs mb-1", isCurrentUser ? "text-right" : "text-left")}>
...
function GroupInfoSidebar({ selectedGroup }) {
  return (
    <div className="p-4 space-y-6">
      {/* Group Description */}
      <div>
        <h3 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-3">About</h3>
      ...
      {selectedGroup.teachers?.length > 0 && (
        <div>
          <h3 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-3">Teachers</h3>
      ...
      {selectedGroup.resources && selectedGroup.resources.length > 0 && (
        <div>
          <h3 className="font-medium text-sm text-muted-foreground uppercase tracking-wide mb-3">Resources</h3>
          <div className="grid gap-3">
            {selectedGroup.resources.map((resource) => (
              <Card key={resource.id} className="overflow-hidden">
                <div className="relative aspect-video bg-muted">
                  <img
                    src={resource.thumbnail || "/placeholder.svg?height=120&width=200"}
                    alt={resource.name}
                    className="object-cover w-full h-full"
                  />
                </div>
                <div className="p-3">
                  <p className="font-medium text-sm truncate mb-2">{resource.name}</p>
